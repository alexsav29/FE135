...
class Contacts {
    ...
    clearLocalStorage() {
        if (!this.getCookie('storageExpiration')) {
            ...
            // window.location.reload(); // !!!! почему, если вызывать перезагрузку здесь или в Конструкторе, страница начинает бесконечно обновляться и ничего не помогает? Куки и localStorage очищаются (нужно обновить страницу и потом повторно обновить, чтобы записи удалились со страницы)
        };

        // for (let i = 0; i <= 2; i++) {   // была идея через цикл, 2 раза обновить страницу, но ничего не получилось - улетает в бесконечную перезагрузку
        //     window.location.reload();
        //     break;
        // };
        // if (!this.getCookie('storageExpiration')) return;
    };
...
class ContactsApp extends Contacts {
    ...
    fetchDataMapper(data) {
        data.forEach((user) => {
            ...
            // console.log({ id, name, email, address: { city, street, suite }, phone });
    ...
    onAdd() {
        ...
        addContactBtn.addEventListener('click', () => {
            ...
            this.renderContactsItems(); // не сразу понял почему здесь тоже нужно вызывать метод отрисовки. Изначально хотел его вызыватьтолько в Конструкторе. Получается его нужно вызывать каждый раз при нажатии на кнопку Добавить контакт (чтобы перерисовывать список контактов в DOM)
