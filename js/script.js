const ToDoList = function () {
    ...
    const createHtmlToDo = () => {
        ...
        // !!! почему-то с тегом form вылетает баг - при добавлении дела, оно на секунду появляется в DOM и пропадает. Так же, если удалить все дела и ввести какое-то дело - оно не появляется, а выводятся те дела, которые были захардкодены.
        // <form class="todo__input__container"><input type="text" class="todo__input" placeholder="Введите задачу" /></form>
        todo.innerHTML = `
    ...
    const render = () => {
        ...
        // без if летят ошибки Uncaught TypeError: Cannot read properties of null (reading 'addEventListener')      at addEditCancel (script.js:167:22)      at render (script.js:109:9)     at HTMLButtonElement.<anonymous> (script.js:175:13)
        // не могу до конца понять почему (оно отрабатывает нормально, но в консоли ошибки при срабатывании события)
    ...
    /* // альтернатива с дополнительной кнопкой
    const addAllToDosDoneEvent = () => {
...
// На написание кода (без стилей) потратил 2 часа 45 минут.